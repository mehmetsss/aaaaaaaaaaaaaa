...
class instaVer extends Command {
    ...
    async run(client, message, args, data) {
        ...
        const embeddd = new Discord.MessageEmbed()
            .setColor('#000001')
            .setAuthor('Yetki falan mı veriyoruz hayırdır?', message.guild.iconURL())
        ...
        collector.on("collect", async (reaction, user) => {
            ...
            switch (reaction.emoji.name) {
                ...
                case "❓":
                    await sexiboy.roles.add(roller.get("cmdregistry").value())
                    break;

                case "🔇":
                    await sexiboy.roles.add(roller.get("cmdmuter").value())
                    break;

                case "🚫":
                    await sexiboy.roles.add(roller.get("cmdjailor").value())
                    break;

                case "⛔":
                    await sexiboy.roles.add(roller.get("cmdbanner").value())
                    break;
