...
class Confirm extends Command {
    ...
    constructor(client) {
        super(client, {
            name: "erkek",
            description: "İsmi ayarladıktan sonra kişiyi erkek olarak kayıt eder",
    ...
    async run(client, message, args, data) {
        ...
        await sexiboy.roles.add([roller.get("erkek").value(), roller.get("member").value(), roller.get("erkeki").value(), roller.get("memberi").value()]);
        ...
        if (!system) {
            try {
                let sex = await namedata({ _id: sexiboy.user.id, isim: isim, yaş: arrs, sex: "Male", date: new Date() });
                await sex.save();
            } catch (error) {
                throw error;
            }
        ...
        message.guild.channels.cache.get((await kanallar).get("cmd_kayıt").value()).send(new Discord.MessageEmbed()
            .setDescription(`${sexiboy} kişisinin verileri başarıyla işlenmiştir.`).setColor('#96e7f4')
