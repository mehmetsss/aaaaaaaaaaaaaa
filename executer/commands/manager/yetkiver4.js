...
class instaVer extends Command {
    ...
    constructor(client) {
        super(client, {
            name: "yetkiver4",
            description: "4. yetkiyi permler ile beraber vermek için kullanılır.",
            usage: "yetkiver4 etiket/id",
            examples: ["yetkiver4 213617765095833600"],
            aliases: [],
    ...
    async run(client, message, args, data) {
        ...
        const verilecek = [roller.get("cmd-vmuter").value(), roller.get("cmd-transport").value(), roller.get("cmd-registry").value(), roller.get("perm4").value(), roller.get("yetkili").value(), roller.get("cmd-cmuter").value()];
        let verilenler = [];
        verilecek.forEach(element => {
            verilenler.push(element)
        });
        await sexiboy.roles.add(verilecek);
        await sexiboy.roles.remove([roller.get("perm3").value(), roller.get("perm2").value(), roller.get("perm1").value()]);
