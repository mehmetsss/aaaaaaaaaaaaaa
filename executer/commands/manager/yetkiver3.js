...
class instaVer extends Command {
    ...
    constructor(client) {
        super(client, {
            name: "yetkiver3",
            description: "3. yetkiyi permler ile beraber vermek için kullanılır.",
            usage: "yetkiver3 etiket/id",
            examples: ["yetkiver3 213617765095833600"],
            aliases: [],
    ...
    async run(client, message, args, data) {
        ...
        const verilecek = [roller.get("cmd-transport").value(), roller.get("cmd-registry").value(), roller.get("perm3").value(), roller.get("yetkili").value(), roller.get("cmd-cmuter").value()];
        let verilenler = [];
        verilecek.forEach(element => {
            verilenler.push(element)
        });
        await sexiboy.roles.add(verilecek);
        await sexiboy.roles.remove([roller.get("perm2").value(), roller.get("perm1").value()]);
