...
class instaVer extends Command {
    ...
    constructor(client) {
        super(client, {
            name: "yetkiver5",
            description: "5. yetkiyi permler ile beraber vermek için kullanılır.",
            usage: "yetkiver5 etiket/id",
            examples: ["yetkiver5 213617765095833600"],
            aliases: [],
    ...
    async run(client, message, args, data) {
        ...
        const verilecek = [roller.get("cmd-vmuter").value(), roller.get("cmd-transport").value(), roller.get("cmd-registry").value(), roller.get("perm5").value(), roller.get("yetkili").value(), roller.get("cmd-cmuter").value(), roller.get("cmd-jailor").value()];
        let verilenler = [];
        verilecek.forEach(element => {
            verilenler.push(element)
        });
        await sexiboy.roles.add(verilecek);
        await sexiboy.roles.remove([roller.get("perm4").value(), roller.get("perm3").value(), roller.get("perm2").value(), roller.get("perm1").value()]);
