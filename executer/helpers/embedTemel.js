...
module.exports = {
    ...
    temel(obj, tit, des) {
        const utiller = low(obj.client.adapterutil);
        const temel = new Discord.MessageEmbed()
            .setAuthor(utiller.get("embed.başlık").value(), utiller.get("tantoony").value())
            .setTitle(tit)
            .setDescription(des)
            .setFooter(`${utiller.get("embed.tarih").value()} | ${new Date(Date.now()).getUTCDate() + 1}.${new Date(Date.now()).getUTCMonth() + 1}.${new Date(Date.now()).getUTCFullYear()}`, obj.member.user.displayAvatarURL())
            .setColor("#2f3136")
        return temel;
    },
    hgmesaj(member, durum) {
        const utiller = low(member.client.adapterutil);
